...
docker run --rm --name filament-db-size-10k -e "NUM_EDGES=10000" -it -v $HOME/benchmark_outputs:/output --link filament-postgres:filament_postgres postgres:9.3 /bin/bash -c 'psql -h $FILAMENT_POSTGRES_PORT_5432_TCP_ADDR $FILAMENT_POSTGRES_PORT_5432_TCP_PORT filament filament -c "select t1.datname AS db_name,  pg_size_pretty(pg_database_size(t1.datname)) as db_size from pg_database as t1 WHERE t1.datname = '"'"'filament'"'"';" > /output/db_size_$NUM_EDGES.txt 2>&1'
